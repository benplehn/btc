...
def build_rainbow_v2(px: pd.DataFrame, extend_to: str | None = None) -> pd.DataFrame:
    ...
    def _lines(dates: Iterable[pd.Timestamp]) -> pd.DataFrame:
        ...
        days = (dates - GENESIS).days.clip(lower=1).astype(float)
...
def main():
    ...
    p.add_argument("--csv", type=str, default=None, help="Chemin d'un CSV local (date, close) si CoinGecko est inaccessible.")
    p.add_argument("--out", type=str, default="outputs/rainbow_v2.png", help="Chemin de sauvegarde du graphique.")
    args = p.parse_args()

    px = load_btc_prices(start=args.start, end=args.end, csv_path=args.csv)
